...
export default function SuperAdminStudentsAdd() {
  ...
  async function submitData(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await request(`${process.env.NEXT_PUBLIC_BACKEND_IP}/superadmin/department/${departmentId}/add`, {method: 'POST', cache: 'no-cache', headers: {'Authorization': `Bearer ${cookie.token}`, 'Content-Type': 'application/json', 'Accept': 'application/json'}, body: JSON.stringify({users: excelFileData})})
      router.push(`/sadmin/departments/${departmentId}`)
      router.refresh()
    } catch(error) {
      console.error(error)

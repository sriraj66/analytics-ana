...
export default function SuperAdminStudentAdd({ params }: { params: { department_id: string } }) {
  ...
  async function submitData(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await request(`${process.env.NEXT_PUBLIC_BACKEND_IP}/superadmin/department/${params.department_id}/add`, {method: 'POST', cache: 'no-cache', headers: {'Authorization': `Bearer ${cookie.token}`, 'Content-Type': 'application/json', 'Accept': 'application/json'}, body: JSON.stringify({users: [formDataObject]})})
      router.push(`/sadmin/departments/${params.department_id}`)
      router.refresh()
    } catch(error) {

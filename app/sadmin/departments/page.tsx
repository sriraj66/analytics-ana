...
export default function SuperAdminCollegeView({ params }: { params: { college_id: string } }) {
  ...
  async function deleteDepartment() {
    ...
    await request(`${process.env.NEXT_PUBLIC_BACKEND_IP}/superadmin/department/${currentDepartment._id}`, {method: 'DELETE', cache: 'no-cache', headers: {'Authorization': `Bearer ${cookie.token}`, 'Content-Type': 'application/json', 'Accept': 'application/json'}})
  ...
  useEffect(() => {
    const fetchDatas = async () => {
      try {
        const response = await request(`${process.env.NEXT_PUBLIC_BACKEND_IP}/superadmin/department`, {method: 'GET', cache: 'no-cache', headers: {'Authorization': `Bearer ${cookie.token}`, 'Content-Type': 'application/json', 'Accept': 'application/json'}})
        const data = await response.json();
        setCollegeName(data.college);
        const grouped_datas = data.department.slice().sort((a: any, b: any) => {
    ...
    <div className={styles.admin_main_contents}>
      <div className={[styles.coloredButton, styles.light_gray_div].join(' ')}>
        <div className={styles.light_gray_heading_div}>
          ...
          <Link href={`/sadmin/departments/add`} className={[styles.iconButton_div, roboto.className].join(' ')}>
                    ...
                    <tbody>
                      {groupedDepartmentDatas[datas].map(data =>
                        <tr key={data._id}>
                          ...
                          <td style={{ padding: 0 }}>
                            <Link href={`/sadmin/departments/${data._id}`} className={[styles.delete_button].join(' ')} key={data._id}>

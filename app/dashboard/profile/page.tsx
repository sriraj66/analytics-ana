...
export default function StudentProfile() {
  ...
  async function profileFormSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await request(`${process.env.NEXT_PUBLIC_BACKEND_IP}/student/settings/${cookie.currentId}/personal`, {method: 'POST', body: JSON.stringify(formDataObject), cache: 'no-cache', headers: {'Authorization': `Bearer ${cookie.token}`, 'Content-Type': 'application/json', 'Accept': 'application/json'}})
    } catch {
  ...
  async function passwordFormSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await request(`${process.env.NEXT_PUBLIC_BACKEND_IP}/student/settings/${cookie.currentId}/security`, {method: 'POST', body: JSON.stringify(formDataObject), cache: 'no-cache', headers: {'Authorization': `Bearer ${cookie.token}`, 'Content-Type': 'application/json', 'Accept': 'application/json'}})
  ...
  async function uploadImage(event: any) {
    ...
    try {
      await request(`${process.env.NEXT_PUBLIC_BACKEND_IP}/student/settings/${cookie.currentId}/personal`, {method: 'POST', body: JSON.stringify({ imageData: profileImg }), cache: 'no-cache', headers: {'Authorization': `Bearer ${cookie.token}`, 'Content-Type': 'application/json', 'Accept': 'application/json'}})
  ...
  async function deleteAvatar(event: any) {
    localStorage.removeItem('userAvatar')
    await request(`${process.env.NEXT_PUBLIC_BACKEND_IP}/student/settings/${cookie.currentId}/personal`, {method: 'POST', body: JSON.stringify({ imageData: '' }), cache: 'no-cache', headers: {'Authorization': `Bearer ${cookie.token}`, 'Content-Type': 'application/json', 'Accept': 'application/json'}})
    document.location.reload()
  }
  ...
  useEffect(() => {
    const fetchDatas = async () => {
      try {
        const response = await request(`${process.env.NEXT_PUBLIC_BACKEND_IP}/student/settings/${cookie.currentId}`, {method: 'GET', cache: 'no-cache', headers: {'Authorization': `Bearer ${cookie.token}`, 'Content-Type': 'application/json', 'Accept': 'application/json'}})
        ...
        <div className={styles2.table_decoration}>
          ...
          <div className={styles.profile_main_div}>
            ...
            <form onSubmit={profileFormSubmit} className={styles.profile_settings_main_div}>
              ...
              <div className={[styles.profile_input_div, poppins.className].join(' ')}>
                <span>College</span>
                <input defaultValue={(profileSettings.college === undefined) ? '' : profileSettings.college} name='college_disabled' disabled={true} type='text' className={poppins.className}></input>
              </div>
              <div className={[styles.profile_input_div, poppins.className].join(' ')}>
                <span>Department</span>
                <input defaultValue={(profileSettings.department === undefined) ? '' : profileSettings.department} name='department_disabled' disabled={true} type='text' className={poppins.className}></input>
              </div>
              <div className={styles.profile_input_div_inline}>
                <div style={{ width: '100%' }} className={[styles.profile_input_div, poppins.className].join(' ')}>
                  <span>Year</span>
                  <input defaultValue={(profileSettings.year === undefined) ? '' : romanize(profileSettings.year)} name='year_disabled' disabled={true} type='text' className={poppins.className}></input>
                </div>
                <div style={{ width: '100%' }} className={[styles.profile_input_div, poppins.className].join(' ')}>
                  <span>Semester</span>
                  <input defaultValue={(profileSettings.semester === undefined) ? '' : romanize(profileSettings.semester)} name='semester_disabled' disabled={true} type='text' className={poppins.className}></input>
                </div>
              </div>
              <div className={styles.profile_input_div_inline}>
                <div style={{ width: '100%' }} className={[styles.profile_input_div, poppins.className].join(' ')}>
                  <span>Section</span>
                  <input defaultValue={(profileSettings.section === undefined) ? '' : profileSettings.section} name='semester_disabled' disabled={true} type='text' className={poppins.className}></input>
                </div>
                <div style={{ width: '100%' }} className={[styles.profile_input_div, poppins.className].join(' ')}>
                  <span>Roll No</span>
                  <input defaultValue={(profileSettings.rollno === undefined) ? '' : profileSettings.rollno} name='rollno' type='text' placeholder='Enter the Roll No' className={poppins.className}></input>
                </div>
              </div>

  ...
  useEffect(() => {
    const fetchDatas = async () => {
      try {
        ...
        exam_data = exam_data.exams.filter((data: examDataArray) => data.category.toLowerCase() === 'mcq').sort((a: examDataArray, b: examDataArray) => (a.status === "ongoing" ? -1 : 1))
      ...
      <div className={[styles.light_white_div].join(' ')}>
        <div className={styles.table_decoration}>
          ...
          {(fetchingData) ? 
            ...
            :
            (examDatas.filter((data) => data.category.toLowerCase() === 'mcq').length === 0) ?

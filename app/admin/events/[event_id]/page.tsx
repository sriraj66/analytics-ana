...
export default function AdminEventView({ params }: { params: { event_id: string } }) {
  ...
  async function deleteEvent() {
    ...
    await request(`${process.env.NEXT_PUBLIC_BACKEND_IP}/admin/event/${eventData._id}`, {method: 'DELETE', cache: 'no-cache', headers: {'Authorization': `Bearer ${cookie.token}`, 'Content-Type': 'application/json', 'Accept': 'application/json'}})
  ...
  useEffect(() => {  
    const fetchDatas = async () => {
      try {
        const response = await request(`${process.env.NEXT_PUBLIC_BACKEND_IP}/admin/event/${params.event_id}`, {method: 'GET', cache: 'no-cache', headers: {'Authorization': `Bearer ${cookie.token}`, 'Content-Type': 'application/json', 'Accept': 'application/json'}})

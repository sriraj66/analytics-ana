...
export default function AdminEventAdd() {
  ...
  async function submitEvent(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await request(`${process.env.NEXT_PUBLIC_BACKEND_IP}/admin/event`, {method: 'POST', cache: 'no-cache', headers: {'Authorization': `Bearer ${cookie.token}`, 'Content-Type': 'application/json', 'Accept': 'application/json'}, body: JSON.stringify(formDataObject)})

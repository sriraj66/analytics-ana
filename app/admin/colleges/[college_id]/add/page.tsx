...
export default function AdminDepartmentAdd({ params }: { params: { college_id: string, department_id: string } }) {
  ...
  async function submitData(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await request(`${process.env.NEXT_PUBLIC_BACKEND_IP}/admin/colleges/${params.college_id}/add`, {method: 'POST', cache: 'no-cache', headers: {'Authorization': `Bearer ${cookie.token}`, 'Content-Type': 'application/json', 'Accept': 'application/json'}, body: JSON.stringify(formDataObject)})
      router.push(`/admin/colleges/${params.college_id}`)
      router.refresh()
    } catch(error) {

...
export default function AdminExamsList() {
  ...
  async function deleteExam() {
    ...
    await request(`${process.env.NEXT_PUBLIC_BACKEND_IP}/admin/exams/${currentExam._id}`, {method: 'DELETE', cache: 'no-cache', headers: {'Authorization': `Bearer ${cookie.token}`, 'Content-Type': 'application/json', 'Accept': 'application/json'}})
    ...
    <div className={styles.admin_main_contents}>
      <div className={[styles.coloredButton, styles.light_gray_div].join(' ')}>
        <div className={styles.light_gray_heading_div}>
          ...
          <Link href='/admin/exams/add' className={[styles.iconButton_div, roboto.className].join(' ')}>
                ...
                <tbody>
                  {examDatas.map(data =>
                    <tr key={data._id}>
                      ...
                      <td style={{ padding: 0 }}>
                        <Link href={`/admin/exams/${data._id}`} className={[styles.delete_button].join(' ')} key={data._id}>

...
export default function AdminExamAdd() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setCollegeFormDatas((collegeFormDatas: collegeDataArray) => ({...collegeFormDatas, college: data.colleges[0]._id}))
      } catch {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setCollegeFormDatas({...collegeFormDatas, department: departmentData.departments[0]._id})
      } catch {
        console.error('Error fetching data');
      }
    };
    ...
    if (excelFile) {
      try {
        readXlsxFile(excelFile).then(data => {
          ...
          if (data[0].length === 8) {
            setExcelFileData(data.slice(1).map((d: Array<any>) => Object({number: d[0], question: d[1], answer: d[2], options: d.slice(3, 7).map(data => data.toString()), rating: d[7]})))
        ...
        <div className={[styles2.exam_div].join(' ')}>
          <div className={[styles2.details_main_div, styles.details_main_div, roboto.className].join(' ')}>
            ...
            <div className={styles2.splitted_options_div}>
              ...
              <div style={{ flexDirection: 'column' }} className={[styles.details_inner_div, styles2.details_inner_div, roboto.className].join(' ')}>
                <span style={{ width: '100%' }} className={[styles2.details_header, styles.details_header].join(' ')}>College Name</span>
                <select onChange={(value) => {setDepartmentList([]); setCollegeId(value.currentTarget.value); setCollegeFormDatas({...collegeFormDatas, college: value.currentTarget.value})}} name='college' className={[styles.details_value, inter.className].join(' ')} required >

...
export default function AdminMeetingView({ params }: { params: { meeting_id: string } }) {
  ...
  async function deleteEvent() {
    ...
    await request(`${process.env.NEXT_PUBLIC_BACKEND_IP}/admin/meeting/${meetingData._id}`, {method: 'DELETE', cache: 'no-cache', headers: {'Authorization': `Bearer ${cookie.token}`, 'Content-Type': 'application/json', 'Accept': 'application/json'}})
  ...
  useEffect(() => {  
    const fetchDatas = async () => {
      try {
        const response = await request(`${process.env.NEXT_PUBLIC_BACKEND_IP}/admin/meeting/${params.meeting_id}`, {method: 'GET', cache: 'no-cache', headers: {'Authorization': `Bearer ${cookie.token}`, 'Content-Type': 'application/json', 'Accept': 'application/json'}})
            ...
            <div className={styles2.event_div}>
              <div className={[styles.details_main_div, roboto.className].join(' ')}>
                ...
                <div className={styles.details_inner_div}>
                  ...
                  <span className={styles.details_value}>{meetingData.college}</span>
                </div>
                <div className={styles.details_inner_div}>

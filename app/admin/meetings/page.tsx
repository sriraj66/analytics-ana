...
export default function AdminMeetingList() {
  ...
  useEffect(() => {  
    const fetchDatas = async () => {
      try {
        const response = await request(`${process.env.NEXT_PUBLIC_BACKEND_IP}/admin/meeting`, {method: 'GET', cache: 'no-cache', headers: {'Authorization': `Bearer ${cookie.token}`, 'Content-Type': 'application/json', 'Accept': 'application/json'}})
    ...
    <div className={styles.admin_main_contents}>
      <div className={styles.light_gray_div}>
        <div className={styles.light_gray_heading_div}>
          ...
          <Link href='/admin/meetings/add' className={[styles.iconButton_div, roboto.className].join(' ')}>
            ...
            meetingDatas.length === 0 ? 
              ...
              <div style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))' }} className={styles.event_main_div}>
                {meetingDatas
                  ...
                  <Link href={`/admin/meetings/${meeting._id}`} style={{ width: '100%', borderTop: '10px solid var(--main-accent)' }} className={[styles2.meeting_div, rubik.className].join(' ')} key={meeting._id}>

...
export default function AdminDashboard() {
  ...
  const [collegeFetchingData, setCollegeFetchingData] = useState(true)
  ...
  useEffect(() => {  
    const fetchDatas = async () => {
      try {
        const response = await request(`${process.env.NEXT_PUBLIC_BACKEND_IP}/admin/exams`, {method: 'GET', cache: 'no-cache', headers: {'Authorization': `Bearer ${cookie.token}`, 'Content-Type': 'application/json', 'Accept': 'application/json'}})
      ...
      try {
        ...
        setCollegeFetchingData(false)
      } catch {
        console.error('Error fetching data');
        setCollegeFetchingData(false)
      }
      try {
        const response = await request(`${process.env.NEXT_PUBLIC_BACKEND_IP}/admin/event`, {method: 'GET', cache: 'no-cache', headers: {'Authorization': `Bearer ${cookie.token}`, 'Content-Type': 'application/json', 'Accept': 'application/json'}})
        let data = await response.json();
    ...
    <div className={styles.admin_main_contents}>
      ...
      <div className={[styles.coloredButton, styles.light_gray_div].join(' ')}>
        <div className={styles.light_gray_heading_div}>
          ...
          <Link href='/admin/exams' className={[styles.iconButton_div, roboto.className].join(' ')}>
            <span>All Exams</span>
          ...
          (examDatas.length === 0) ?
            ...
            <div style={{ gap: '6px', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))' }} className={styles.event_main_div}>
              {examDatas.map((exam: examDataArray) =>
                <Link href={`/admin/exams/${exam._id}`} style={{ width: '100%', borderLeft: '10px solid var(--navy)' }} className={[styles.exam_main_div, rubik.className].join(' ')} key={exam._id}>
      ...
      <div style={{ padding: '16px' }} className={[styles.coloredButton, styles.light_gray_div].join(' ')}>
        ...
        <div className={styles.details_list_div}>
          {(collegeFetchingData) ? 
            <span style={{ padding: '15px' }} className={[styles.no_datas].join(' ')}>
              <Spinner color='#1b238d' size={45} />
            </span>
            ...
            collegeDatas.length === 0 ? 
              <span className={[styles.no_datas, roboto.className].join(' ')} >
                Currently there are No Colleges
              </span> :
              collegeDatas.map((college: collegeDataArray) => 
                <Link href={`/admin/colleges/${college._id}`} className={[styles.college_list, rubik.className].join(' ')} key={college._id}>
                  <BiSolidInstitution style={{ color: 'white', backgroundColor: '#282b4a', padding: '4px', borderRadius: '6px', fontSize: '25px' }} />
                  {college.college}
                </Link>
                )}
        </div>
      ...
      <div className={[styles.coloredButton, styles.light_gray_div].join(' ')}>
        <div className={styles.light_gray_heading_div}>
          ...
          <Link href='/admin/events' className={[styles.iconButton_div, roboto.className].join(' ')}>
            ...
            eventDatas.length === 0 ? 
              ...
              <div className={styles.event_main_div}>
                {eventDatas.map((event: eventDataArray) => 
                  <Link href={`/admin/events/${event._id}`} className={styles.event_div} key={event._id}>
